...
def test_ragas_path_uses_embeddings_and_ground_truths(monkeypatch):
    ...
    # 同时 mock _build_llm_judge，避免 _judge_sample 去调用真实 API
    class FakeLLMResponse:
        content = '{"overall_score": 0.8, "correctness_score": 0.75, "completeness_score": 0.8, "relevance_score": 0.9, "grounding_score": 0.7, "reasoning_score": 0.7, "step_coherence_score": 0.7, "faithfulness_score": 0.7, "helpfulness_score": 0.8, "context_recall": 0.8, "reason": "mock"}'

    class FakeLLM:
        def invoke(self, prompt):
            return FakeLLMResponse()

    monkeypatch.setattr(evaluation_methods_module, "_build_llm_judge", lambda: FakeLLM(), raising=False)
