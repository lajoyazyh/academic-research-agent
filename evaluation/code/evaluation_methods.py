...
class _ZhipuCompatibleChatOpenAI(ChatOpenAI):
    ...
    def __init__(self, **kwargs: Any):
                ...
                for msg in kw["messages"]:
                    if isinstance(msg, dict):
                        ...
                        for mbk in msg_bad:
                            msg.pop(mbk, None)
            try:
                result = orig_create(*args, **kw)
                # 兼容性 shim：旧版 openai SDK 返回的 ChatCompletion 没有 .parse()，
                # 但新版 langchain_openai 会调用它。此处动态添加。
                if not hasattr(result, "parse"):
                    def _parse_result():
                        return result
                    setattr(result, "parse", _parse_result)
                return result
        ...
        async def patched_acreate(*args, **kw):
            ...
            try:
                result = await orig_acreate(*args, **kw)
                # 兼容性 shim：同上
                if not hasattr(result, "parse"):
                    def _parse_result():
                        return result
