...
搜索阶段由 \texttt{BaseAgent} 执行。每轮要求模型输出 JSON 格式的 \texttt{thought/action/action\_input}，工具返回写入 \texttt{observation}。
...
\subsection{深度分析}
分析阶段生成三类结果：文献对比、研究脉络、研究空白。结果保存到 \texttt{analysis/analysis\_results.json}，前端以三张 Markdown 卡片展示并允许编辑。
