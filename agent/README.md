...
| `paper_register` | 内部工具 | 审核摘要 → 收录论文，一步完成（下载 PDF + 登记到论文列表） |
...
| 工具 | 功能 |
|------|------|
| `RAGNoteGenerator` | 基于 Embedding 向量检索的深度笔记生成器：PDF 全文 → 分块 → 向量检索 Top-K 段落 → LLM 逐节生成结构化笔记 |
| `retriever` | BM25 检索器：为对话栏提供论文 PDF 段落检索，注入问答/修订 prompt |
| `clear_notes` / `append_note` | 研究笔记管理（CLI 模式使用） |
...
全程实时显示进度（状态栏 + 轨迹视图），完成后自动切换到综述视图。支持随时点击「停止」取消。
...
- ✏️ **Vditor Markdown 编辑器** — 在线编辑笔记和综述
- 📝 **AI 判意修订** — 对话区隐式修改意图先判定再确认执行
- 🛑 **检索打断** — 搜索进行中可随时停止
- 📊 **实时轨迹视图** — 搜索进行中即可查看每一步的思考/行动/观察
- 🚀 **一键自动模式** — 规划→搜索→笔记→综述全自动执行
...
| POST | `/api/sessions/{id}/run/auto` | 🚀 一键自动模式（规划→搜索→笔记→综述） |
