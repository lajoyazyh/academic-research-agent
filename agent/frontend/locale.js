(function () {
  ...
  var exact = {
    ...
    "篇综述": "reviews",
    "个项目": "projects",
    "此处仅用于身份登录，不请求仓库读写权限。需要调研或导出仓库时，可在个人中心单独授权。": "This sign-in only verifies your identity and does not request repository access. Authorize repository access separately in Profile when you need repository research or export.",
    "重新加载模型": "Reload models",
    "当前阶段": "Current stage",
    "检索高级设置": "Advanced search settings",
    "请选择研究项目": "Select a research project",
    "从一个研究问题开始": "Start with a research question",
    "创建项目后，这里会显示论文摘要、笔记和 PDF 原文。": "Paper abstracts, notes, and full-text PDFs will appear here after you create a project.",
    "还没有打开研究项目。": "No research project is open.",
    "任务已中断，可重试": "Run interrupted · retry available",
    "正在从最近的检查点重新启动任务...": "Restarting the run from its latest checkpoint...",
    "任务正在运行，已恢复进度跟踪...": "The run is active. Progress tracking has been restored...",
    "已通过质量门禁": "Quality gate passed",
    "建议复核后导出": "Review before export",
    "查看质量风险": "View quality risks",
    "优先补齐全文或删除无法由来源支持的表述，再重新生成综述。": "Add the missing full text or remove statements that the sources cannot support, then regenerate the review.",
    "本轮检索覆盖": "Search coverage for this run",
    "本轮新增": "Added this run",
    "实际查询": "Queries issued",
    "尝试登记": "Registration attempts",
    "重复候选": "Duplicate candidates",
    "暂无来源调用记录": "No source calls recorded",
    "前往个人中心配置模型": "Open Profile to configure a model",
    "手机工作台导航": "Mobile workspace navigation",
    "点击「搜索相关论文」或「添加论文」开始构建来源库。": "Search for papers or add a paper to start building the source library.",
    "请先选择“纳入综述”": "Include at least one paper first",
    "关键词已确认，点击「AI检索论文」开始搜索": "Keywords confirmed. Select AI paper search to begin.",
    "还没有综述草稿。选中论文并生成笔记后，点击「生成综述」按钮生成。": "No review draft yet. Include papers, generate notes, then select Generate review.",
    "未生成": "Not generated",
    "📝 综述": "📝 Review",
    "本轮检索设置": "Search settings for this run",
    "一键自动完成：规划、检索、笔记、分析和综述": "Run planning, search, notes, analysis, and review automatically",
    "检索词已确认": "Search terms confirmed",
    "未选择论文": "No paper selected",
    "请先在左侧选择一篇论文。": "Select a paper from the left panel first.",
    "默认对话": "Default conversation",
    "点击左侧论文进入对应界面后可在下方提问，笔记无需勾选，综述模式需要勾选对应的论文。笔记模式和综述模式下可请求修改。": "Open a paper from the left panel, then ask questions below. Notes use the current paper; review mode uses the included papers. You can request revisions in Notes and Review modes.",
    "对话模式": "Chat mode",
    "Agent 模式": "Agent mode",
    "登录服务未配置，当前只能查看模型目录。": "Sign-in is not configured; the model catalog remains available in read-only mode.",
    "预设提供商会自动填写；自定义接口需兼容 OpenAI API。": "Preset providers are filled automatically; custom endpoints must be OpenAI API-compatible.",
    "请输入你的 API Key": "Enter your API key",
    "无笔记": "No notes",
    "最近活动": "Recent activity",
    "对话历史": "Conversation history",
    "你好！我是全局 Copilot，可以基于你所有研究项目的论文、笔记和综述来回答问题。": "Hi! I am the workspace Copilot. I can answer questions using papers, notes, and reviews across your research projects.",
    "试试问我：": "Try asking:",
    "关闭 Copilot": "Close Copilot",
    "新建对话": "New conversation",
    "选择全部项目": "Select all projects",
    "向 Copilot 提问（跨项目知识问答）...": "Ask Copilot across your research projects...",
    "重建知识库索引": "Rebuild knowledge index",
    "关键词已确认": "Keywords confirmed",
    "进入": "Open"
  };
  ...
  var patterns = [
    ...
    [/^· (\d+) 分钟前$/, "· $1 minutes ago"],
    [/^(\d+) 分钟前$/, "$1 minutes ago"],
    [/^最近活跃 · (.+) · (\d+) 分钟前$/, "Recently active · $1 · $2 minutes ago"],
    [/^基于全部 (\d+) 个 Session 的论文、笔记、综述回答$/, "Answer using papers, notes, and reviews from all $1 projects"],
    [/^(\d+) 篇（(\d+) 有笔记）$/, "$1 papers ($2 with notes)"],
    [/^(\d+) 个来源 · 摘要 · 检索词已确认$/, "$1 sources · Summary · Search terms confirmed"],
    [/^(\d+) 个来源 · (.+) · 检索词已确认$/, "$1 sources · $2 · Search terms confirmed"],
    [/^(\d+) \/ (\d+) tokens · (\d+) 轮$/, "$1 / $2 tokens · $3 turns"],
    [/^(\d+) 个$/, "$1 items"],
    [/^已启用: (\d+)$/, "Enabled: $1"],
    [/^已禁用: (\d+)$/, "Disabled: $1"],
    ...
    [/^目标 (\d+) 篇；最多执行 (\d+) 轮模型决策。$/, "Target: $1 papers; up to $2 model-decision rounds."],
    [/^目标 (\d+) 篇；最多执行 (\d+) 轮模型决策，达到目标后会提前结束。$/, "Target: $1 papers; up to $2 model-decision rounds, stopping early once the target is reached."],
    [/^证据质量 (\d+)\/100$/, "Evidence quality $1/100"],
    [/^来源覆盖 (\d+)%$/, "Source coverage $1%"],
    [/^论断覆盖 (\d+)%$/, "Claim coverage $1%"],
    [/^缺少引用的实质论断：(\d+) 处$/, "Substantive claims without citations: $1"],
    [/^无法验证的引用：(.*)$/, "Unverifiable citations: $1"],
    [/^仅基于摘要的来源：(.*)$/, "Abstract-only sources: $1"],
