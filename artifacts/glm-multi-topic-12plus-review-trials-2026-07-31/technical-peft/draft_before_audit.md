...
## 摘要
随着大语言模型（LLM）规模的指数级增长，全参数微调（Full Fine-tuning）在计算资源与存储成本上构成了严峻挑战。参数高效微调（PEFT）通过冻结预训练模型的大部分参数，仅更新少量附加参数，从而在保持模型性能的同时大幅降低资源消耗。本文基于对12篇arXiv论文的系统综述，构建了PEFT方法的技术谱系，涵盖加性微调、重参数化微调、混合微调及基于量化的微调四大类。研究发现，LoRA和Houlsby Adapter是当前最主流且性能最稳定的方法，能在极低参数量（<1%）下达到全微调性能；QLoRA通过量化技术将65B模型的训练内存需求降低至48GB以下，实现了单卡训练的可能性；AdaMix等混合方法在特定任务（如GLUE）上超越了全微调。然而，Prompt Tuning等方法在收敛速度和稳定性上存在显著局限，且GPT-4评估系统存在偏差。本文进一步分析了不同方法在可训练参数规模、结构改动、任务适用性、训练与推理成本及复现性方面的工程权衡，为PEFT技术的选型与应用提供了实证依据。
...
## 参考来源
[P1] Parameter-efficient fine-tuning of large-scale pre-trained language models (Delta Tuning)
[P2] Delta Tuning: A Comprehensive Study of Parameter Efficient Methods for Pre-trained Language Models
[P3] LLM-Adapters: An Adapter Family for Parameter-Efficient Fine-Tuning of Large Language Models
[P4] LLaMA-Adapter: Efficient Fine-tuning of Language Models with Zero-init Attention
[P5] QLoRA: Efficient Finetuning of Quantized LLMs
[P6] Scaling Down to Scale Up: A Guide to Parameter-Efficient Fine-Tuning
[P7] Parameter-Efficient Fine-Tuning Methods for Pretrained Language Models: A Critical Review and Assessment
[P8] Parameter-Efficient Fine-Tuning for Large Models: A Comprehensive Survey
[P9] One-for-All: Generalized LoRA for Parameter-Efficient Fine-Tuning
[P10] AdaMix: Mixture-of-Adaptations for Parameter-efficient Model Tuning
[P11] FedPETuning: When Federated Learning Meets the Parameter-Efficient Tuning Methods of Pre-trained Language Models
[P12] Heterogeneous LoRA for Federated Fine-tuning of On-Device Foundation Models
