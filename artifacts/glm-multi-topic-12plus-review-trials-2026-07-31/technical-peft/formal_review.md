...
## 摘要
随着大语言模型（LLM）规模增长，全参数微调在计算资源与存储成本上构成了严峻挑战。参数高效微调（PEFT）通过冻结预训练模型的大部分参数，仅更新少量附加参数，从而降低资源消耗[P1]。本轮聚焦技术综述基于12篇可读取全文的论文构建PEFT方法谱系，涵盖加性微调、重参数化微调、混合微调及基于量化的微调。纳入证据显示，LoRA和Houlsby Adapter可在较低可训练参数比例下接近全参数微调性能[P1][P6]；QLoRA通过量化降低大模型微调的显存需求[P5]；AdaMix等混合方法在特定任务上报告了相对全参数微调的增益[P10]。本文进一步比较不同方法在可训练参数规模、结构改动、任务适用性、训练与推理成本及复现性方面的工程权衡。

## 引言
预训练语言模型的参数规模使全参数微调面临较高的计算与存储成本[P1][P6]。参数高效微调（PEFT）通常冻结大部分预训练权重，并通过新增参数或受限权重更新适配下游任务[P1][P6]。纳入语料覆盖 Adapter、Prefix Tuning、LoRA 及其衍生方法。
...
## 方法
【方法记录】本轮通过 OpenAlex 执行三条预设主题查询，形成47条去重候选记录；经标题/摘要高召回排序和开放全文可得性验证，纳入12篇可解析全文。证据提取采用结构化证据卡，并按方法族、性能与成本、评测公平性和复现性组织综合单元。该流程用于高负载工作流验证，不是穷尽性系统综述。

【方法记录】
1.  **文献检索**：【方法记录】在 OpenAlex 分别执行 PEFT/LoRA/Adapter、低秩与量化微调、Prompt/Prefix/Adapter 三组英文查询；检索式与命中数保存在检索账本中。
2.  **筛选流程**：
    *   **初步筛选**：根据标题和摘要，排除非技术性文章、非PEFT相关文章（如纯理论或纯应用文章）。
    *   **全文筛选**：筛选出的候选论文需提供可读取的全文，且必须包含PEFT方法的机制描述、实验设置及结果。
    *   **纳入标准**：【方法记录】最终纳入12篇可解析全文，涵盖方法论文、技术报告、实证研究及综述文章。
3.  **证据提取**：从每篇论文中提取关于方法机制、实验设置、性能指标、资源消耗及复现性的结构化信息。
...
### 2. 可训练参数与结构改动
不同PEFT方法报告的可训练参数比例和结构改动不同，因此部署成本需结合模型、任务与实现分别比较[P1][P6][P7]。
...
### 4. 训练与推理成本
纳入研究普遍报告PEFT降低训练期显存占用，但幅度依赖模型规模和实验设置[P1][P6]。在小批量（1和8）下，Delta Tuning方法报告节省高达3/4的GPU内存；在大批量（32和64）下报告节省约1/2到1/3[P1]。QLoRA通过量化进一步降低显存需求，并演示了65B模型的单卡微调[P5]。
...
然而，部分方法存在显著的不确定性。Prompt Tuning的性能严重依赖随机种子，方差较大，需要多次运行取平均[P6]。[P5]指出，GPT-4评估系统存在顺序效应和自评偏差，导致模型性能评估可能被高估（如Guanaco 65B的自评Elo为1348，而人类评估为1176）[P5]。此外，HETLORA等针对联邦学习的PEFT方法的理论收敛性和泛化性尚未得到充分证明[P12]。
...
### 本综述的局限
本研究只从47条 OpenAlex 候选中纳入12篇可获取全文，可能遗漏未被查询命中、无开放全文或位于其他索引中的重要工作，因此不能宣称系统性穷尽覆盖。不同论文使用的评估指标、数据集和模型规模存在差异，直接比较存在困难。[P5]使用的模型评估方式也需要结合其原始实验设置谨慎解释[P5]。

### 研究与实践启示
【作者综合判断】实践选型应按任务、硬件、延迟和复现要求分别验证，不能从本轮异质实验中推出单一“最佳方法”。纳入综述显示 PEFT 已被讨论用于语言、视觉、视觉—语言动作与扩散模型，但跨模态的统一比较仍不足[P8]。联邦或边缘场景还需检验客户端异构性与数据分布相关时的稳健性[P12]。后续研究可优先报告多随机种子、推理延迟和标准化基准结果[P6][P7]。
