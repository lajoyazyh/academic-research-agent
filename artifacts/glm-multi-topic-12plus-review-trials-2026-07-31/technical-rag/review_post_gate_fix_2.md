...
不同研究在评估指标和实验设置上的异质性是理解证据边界的关键。P2和P5主要依赖EM和准确率等传统指标，而P4、P6和P10则强调引用准确率、忠实度和鲁棒性等RAG特有指标。这种差异导致直接比较不同研究的性能排名变得困难。例如，P2声称的8.6%绝对增益是基于EM指标，而P8在CRAG中报告的20%+提升是基于准确率和FactScore。因此，在解读“SOTA”性能时，必须明确其适用的指标和任务场景 `[P2]` `[P8]`。
