...
## 摘要
本文综述了大型语言模型（LLM）智能体在规划、工具使用、记忆、协作及长期任务能力方面的评价现状。现有研究构建了涵盖通用推理、工具调用及多模态交互的多种基准测试，并提出了包括任务成功率、人类反馈及 LLM-as-a-Judge 在内的多维评估指标。然而，当前评估体系在可复现性、数据偏差、环境封闭性及长期一致性方面仍面临显著挑战。
