...
## 摘要
随着大语言模型（LLM）从单一文本生成向具备自主决策能力的智能体演进，如何科学、全面地评价其规划、工具使用、记忆、协作及长期任务能力成为关键挑战。本文综述了相关研究，分析了现有评价体系的局限性及未来方向。研究发现，现有的基准测试在任务设计、指标选择、可复现性及有效性风险方面存在显著差异，且在长期记忆管理、多 Agent 协作以及真实世界动态环境下的评估仍存在不足。

## 引言
LLM Agent 的评价通常围绕其核心架构模块展开，包括规划、记忆、工具使用及协作能力。现有研究指出，LLM 具备内在的推理能力，能够通过 CoT（思维链）进行任务分解，但在复杂的多步任务中，单纯依赖模型内在推理存在局限性，往往需要引入外部规划工具或机制来增强鲁棒性。工具使用能力是 Agent 赋能现实世界的关键，评价通常关注“何时调用”、“调用哪个工具”以及“如何使用工具”三个维度。针对长期任务，现有基准如 AgentDojo、FlowBench 等开始探索多轮交互和动态环境下的评估，但大多数基准仍侧重于单次交互或短周期任务，难以捕捉 Agent 在长时间运行中的性能漂移或累积效应。

## 方法
本研究通过系统检索与筛选，纳入了 12 篇相关文献，包括 10 篇综述和 2 篇基准测试论文。文献检索主要聚焦于 LLM 智能体的架构设计、能力评估框架及基准测试方法。筛选过程遵循以下原则：
...
【方法记录】
-   **检索策略**：基于关键词“LLM Agent”、“Evaluation”、“Benchmark”、“Planning”、“Tool Use”、“Memory”进行文献检索。
-   **筛选过程**：剔除与智能体评价无关的通用 LLM 评估或纯算法改进论文，保留专注于智能体能力评估与基准构建的文献。
-   **纳入来源**：最终纳入 [P1]-[P12] 共 12 篇文献，其中 [P1], [P2], [P3], [P4], [P5], [P6], [P7], [P9], [P10], [P11] 为综述，[P8], [P12] 为基准/评论。
